...
var bot_token = '1971680193:AAHLVf6eC3PoXrH8-gmLAXp6gU30DTWV7Jk'; //YOUR BOT TOKEN HERE
    ...
    try {
        ...
        if (bots == 'Active') {
            ...
            if (data.length == 0 && ctx.from.id != +ctx.match[1]) { //IF USER IS NOT IN DATA
                db.collection('allUsers').insertOne({ userID: ctx.from.id, balance: 0.00, toWithdraw: 0.00,verified:false })
                db.collection('wallet').insertOne({ userID: ctx.from.id })
      ...
      let admin = await db.collection('admindb').find({ admin: "admin" }).toArray()
        ...
        if (bots == 'Active') {
            ...
            if (flag == channel.length) {
                ctx.replyWithMarkdown(
                    '*🏡 Welcome To Main Menu*', { reply_markup: { keyboard: [['💰 Balance','📘 Daily Quiz'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['📤 Payout','📊 Status','🏦 More']], resize_keyboard: true } }
                )
...
bot.hears('💰 Balance', async (ctx) => {
    try {
        ...
        if (bots == 'Active') {
            ...
            if (flag == channel.length) {
                ctx.replyWithMarkdown(
                    '*🙌🏻 User = ' + ctx.from.first_name + '\n\n💰 Balance = ' + ub.toFixed(3) + ' ' + currency + '\n\n🪢 Invite To Earn More*', { reply_markup: { keyboard: [['💰 Balance','📘 Daily Quiz'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['📤 Payout','📊 Status','🏦 More']], resize_keyboard: true } }
                )
            } else {
                mustjoin(ctx)
            }
...
bot.hears('🙌🏻 Invite', async (ctx) => {
    try {
        ...
        if (bots == 'Active') {
            ...
            if (flag == channel.length) {
                ctx.replyWithMarkdown(
                    '*🙌🏻 User =* [' + ctx.from.first_name + '](tg://user?id=' + ctx.from.id + ')\n\n*🙌🏻 Your Invite Link = https://t.me/' + ctx.botInfo.username + '?start=' + ctx.from.id + ' \n\n🪢 Invite To ' + refer + ' ' + currency + ' Per Invite*', { reply_markup: { keyboard: [['💰 Balance','📘 Daily Quiz'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['📤 Payout','📊 Status','🏦 More']], resize_keyboard: true } }
                )
            } else {
                mustjoin(ctx)
            }
        } else {
            ctx.replyWithMarkdown('*⛔ Bot Is Currently *')
        }
    } catch (error) {
        console.log(error)
    }
        ...
        if (bots == 'Active') {
            ...
            if (flag == channel.length) {
                ...
                if (('inviter' in userdata[0]) && !('referred' in config[0])) {
                  ...
                  ctx.replyWithMarkdown("*❌Must Verify Yourself*")
                }
            } else {
                mustjoin(ctx)
            }
        ...
        if (bots == 'Active') {
            if (withs == 'ON') {
                ...
                if (flag == channel.length) {
                    ...
                    if (ub < mini_with) {
                        ctx.replyWithMarkdown(
                            '*⚠️ Must Own AtLeast ' + mini_with + ' ' + currency + ' To Make Withdrawal*', { reply_markup: { keyboard: [['💰 Balance','📘 Daily Quiz'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['📤 Payout','📊 Status','🏦 More']], resize_keyboard: true } }
                        )
                    ...
                    if (duration_in_hours >= 24) {
                      ...
                      ctx.reply(
                        '*⛔ You Already Made A Withdrawal In Last 24 Hours *',{parse_mode:'markdown'})
                    }
                    }
                } else {
                    mustjoin(ctx)
...
bot.hears("📘 Daily Quiz", async(ctx) => {
  try {
    let bdata = await db.collection('PollUsers').find({ userID: ctx.from.id }).toArray()
    var duration_in_hours;
    var time = new Date().toISOString();
    if (bdata.length == 0) {
       db.collection('PollUsers').insertOne({ userID: ctx.from.id, pollbonus: new Date() })
       duration_in_hours = 24;
    } else {
       duration_in_hours = ((new Date()) - new Date(bdata[0].pollbonus)) / 1000 / 60 / 60;
    }
    if (duration_in_hours >= 24) {
       db.collection('PollUsers').updateOne({ userID: ctx.from.id }, { $set: { pollbonus: time } }, { upsert: true })
        bot.telegram.sendPoll(ctx.from.id,"✌️In What Language Is This Bot Coded In?",["PHP","NODE.JS","PYTHON"],{is_anonymous:false,type:"quiz",correct_option_id:1}).catch(err=> sendError(err,ctx))
      } else {
        ctx.reply(
          '*⛔ You Already Completed Quiz In Last 24 Hours *',{parse_mode:'markdown'}
          )
      }
  } catch(err) {
    console.log(err)
...
bot.on("poll_answer",async(ctx) => {
  try{
   let ans = ctx.update.poll_answer.option_ids[0]
   let to = ctx.update.poll_answer.user.id
   if (ans == "1"){
      let userbal = await db.collection('balance').find({ userID: to }).toArray()
       var cur = userbal[0].balance * 1
       var balance = cur + 0.5
       db.collection('balance').updateOne({ userID: to }, { $set: { balance: balance } }, { upsert: true })
       bot.telegram.sendMessage(to,"*🎁Congrats, You Recieved 0.5 INR*",{parse_mode:'markdown'})
   } else {
     bot.telegram.sendMessage(to,"*❌ Better Luck Next Time *",{parse_mode:'markdown'})
   }
  } catch(err){
    console.log(err)
  }
        ...
        if (bots == 'Active') {
            ...
            if (flag == channel.length) {
                ...
                if (statdata.length == 0) {
                    ...
                    ctx.reply(
                        '<b>📊 Bot Live Stats 📊\n\n📤 Total Payouts : 0 ' + currency + '\n\n💡 Total Users: ' + members.length + ' Users</b>' , { parse_mode: 'html', reply_markup: { keyboard: [['💰 Balance','📘 Daily Quiz'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['📤 Payout','📊 Status','🏦 More']], resize_keyboard: true } }
                    )
                } else {
                    let payout = statdata[0].value * 1
                    let memb = parseInt(members.length)
                    ctx.reply(
                        '<b>📊 Bot Live Stats 📊\n\n📤 Total Payouts : ' + payout + ' ' + currency + '\n\n💡 Total Users: ' + memb + ' Users</b>', { parse_mode: 'html', reply_markup: { keyboard: [['💰 Balance','📘 Daily Quiz'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['📤 Payout','📊 Status','🏦 More']], resize_keyboard: true } }
                    )
                }
            } else {
                mustjoin(ctx)
...
bot.hears('/adminhelp', async (ctx) => {
    try {
        ...
        if (ctx.from.id == 827167974 || ctx.from.id == 1117956586) {
...
broadcast.hears(regex, async (ctx) => {
    try {
        if (ctx.message.text == '⛔ Cancel') {
            ...
            ctx.replyWithMarkdown(
                '*📣 Broadcast Sent To: ' + users.length + ' Users*', { reply_markup: { keyboard: [['💰 Balance','📘 Daily Quiz'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['📤 Payout','📊 Status','🏦 More']], resize_keyboard: true } }
            )
...
wallet.on('text', async (ctx) => {
    try {
        let admin = await db.collection('admindb').find({ admin: "admin" }).toArray()
        let wal = await db.collection('wallet').find({ wallet: ctx.message.text }).toArray()
        let channel = admin[0].channels
        var flag = 0;
        for (i in channel) {
            let res = await bot.telegram.getChatMember(channel[i], ctx.from.id)
        ...
        if (flag == channel.length) {
            if (ctx.message.text == '⛔ Cancel') {
              ...
              if(wal.length == 0){
                ...
                db.collection('wallet').updateOne({ userID: ctx.from.id }, { $set: { wallet: ctx.message.text } }, { upsert: true })
                ctx.replyWithMarkdown(
                    '*🗂 Wallet Address Set To: *\n`' + ctx.message.text + '`', { reply_markup: { keyboard: [['💰 Balance','📘 Daily Quiz'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['📤 Payout','📊 Status','🏦 More']], resize_keyboard: true } }
                )
              } else {

              ctx.reply(

                    '<b>❌ This Number Is Already In Use By <a href="tg://user?id='+wal[0].userID+'">'+wal[0].userID+'</a></b>', {parse_mode: 'html', reply_markup: { keyboard: [['💰 Balance','📘 Daily Quiz'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['📤 Payout','📊 Status','🏦 More']], resize_keyboard: true } }
            ...
            if (flag == channel.length) {
                ...
                if (ctx.message.text == '⛔ Cancel'){
                  ctx.replyWithMarkdown(
                        ...
                        '*⛔ Withdrawal Cancelled*', { reply_markup: { keyboard: [['💰 Balance','📘 Daily Quiz'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['📤 Payout','📊 Status','🏦 More']], resize_keyboard: true } }
                    ...
                    ctx.replyWithMarkdown(
                        '*⛔ Only Numeric Value Allowed*', { reply_markup: { keyboard: [['💰 Balance','📘 Daily Quiz'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['📤 Payout','📊 Status','🏦 More']], resize_keyboard: true } }
                    )
                    ctx.scene.leave('onWithdraw')
                    return
                } else if (ctx.message.text > ub) {
                    ctx.replyWithMarkdown(
                        '*⛔ Entered Amount Is Greater Than Your Balance*', { reply_markup: { keyboard: [['💰 Balance','📘 Daily Quiz'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['📤 Payout','📊 Status','🏦 More']], resize_keyboard: true } }
                    )
                    ctx.scene.leave('onWithdraw')
                    return
                    ...
                    ctx.replyWithMarkdown(
                        ...
                        '*⚠️ Minimum Withdrawal Is ' + mini_with + ' ' + currency + '*', { reply_markup: { keyboard: [['💰 Balance','📘 Daily Quiz'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['📤 Payout','📊 Status','🏦 More']], resize_keyboard: true } }
...
conf.hears("✅ Approve", async (ctx) => {
    ...
    var userbalance = await db.collection('balance').find({ userID: ctx.from.id }).toArray()
...
tgid.hears(/^[0-9]+$/, async (ctx) => {
    try {
        if (ctx.message.text == '⛔ Cancel') {
            ...
            if (!data[0]) {
                ctx.replyWithMarkdown(
                    '*⛔ User Is Not Registered In Our Database *', { reply_markup: { keyboard: [['💰 Balance','📘 Daily Quiz'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['📤 Payout','📊 Status','🏦 More']], resize_keyboard: true } }
                )
            } else {
                ...
                ctx.reply(
                    '<b>🫂 User : <a href="tg://user?id=' + ctx.message.text + '">' + ctx.message.text + '</a>\n⛔ User Id</b> : <code>' + ctx.message.text + '</code>\n\n<b>💰 Balance : ' + bal + '\n🗂 Wallet : </b><code>' + add + '</code>\n<b>🙌🏻 Inviter : </b><code>' + invite + '</code>', { parse_mode: 'html', reply_markup: { keyboard: [['💰 Balance','📘 Daily Quiz'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['📤 Payout','📊 Status','🏦 More']], resize_keyboard: true } }
                )
            }
        }
...
incr.hears(regex, async (ctx) => {
    try {
        if (ctx.message.text == '⛔ Cancel') {
            ...
            ctx.reply(
                '<b>💰 Balance Of <a href="tg://user?id=' + user + '">' + user + '</a> Was Increased By ' + amount + '\n\n💰 Final Balance = ' + final + '</b>', { parse_mode: 'html', reply_markup: { keyboard: [['💰 Balance','📘 Daily Quiz'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['📤 Payout','📊 Status','🏦 More']], resize_keyboard: true } }
            )
...
chnl.hears(regex, async (ctx) => {
    try {
        ...
        if (ctx.message.text == '⛔ Cancel') {
            ...
            ctx.reply(
                '<b>🗂 Channel Added To Bot : ' + ctx.message.text + '</b>', { parse_mode: 'html', reply_markup: { keyboard: [['💰 Balance','📘 Daily Quiz'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['📤 Payout','📊 Status','🏦 More']], resize_keyboard: true } }
            )
        } else {
            ctx.replyWithMarkdown(
                '*⛔ Channel User Name Must Start With "@"*', { reply_markup: { keyboard: [['💰 Balance','📘 Daily Quiz'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['📤 Payout','📊 Status','🏦 More']], resize_keyboard: true } }
            )
        }
    ...
    try {
        ...
        if (ctx.message.text == '⛔ Cancel') {
            ...
            if (contains("" + ctx.message.text + "", chan)) {
                ...
                ctx.reply(
                    '<b>🗂 Channel Removed From Bot : ' + ctx.message.text + '</b>', { parse_mode: 'html', reply_markup: { keyboard: [['💰 Balance','📘 Daily Quiz'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['📤 Payout','📊 Status','🏦 More']], resize_keyboard: true } }
                )
            } else {
                ctx.reply(
                    '<b>⛔ Channel Not In Our Database</b>', { parse_mode: 'html', reply_markup: { keyboard: [['💰 Balance','📘 Daily Quiz'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['📤 Payout','📊 Status','🏦 More']], resize_keyboard: true } }
                )
            }
        } else {
            ctx.replyWithMarkdown(
...
paychnl.hears(regex, async (ctx) => {
    try {
        ...
        if (ctx.message.text == '⛔ Cancel') {
            ...
            ctx.reply(
                '<b>🗂 Pay Channel Set To : ' + ctx.message.text + '</b>', { parse_mode: 'html', reply_markup: { keyboard: [['💰 Balance','📘 Daily Quiz'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['📤 Payout','📊 Status','🏦 More']], resize_keyboard: true } }
            )
        } else {
            ctx.replyWithMarkdown(
                '*⛔ Channel User Name Must Start With "@"*', { reply_markup: { keyboard: [['💰 Balance','📘 Daily Quiz'], ['🙌🏻 Invite', '🎁 Bonus', '🗂 Wallet'], ['📤 Payout','📊 Status','🏦 More']], resize_keyboard: true } }
...
bot.action('withstat', async (ctx) => {
    try {
        ...
        if (ctx.from.id == 827167974 || ctx.message.text == 1117956586) {
            ctx.editMessageText("<b>🏡 Hey " + ctx.from.first_name + "\n🤘🏻 Welcome To Admin Panel\n\n💡 Bot Current Stats: \n\t\t\t\t📛 Bot : @" + ctx.botInfo.username + "\n\t\t\t\t🤖 Bot Status: " + botstt + "\n\t\t\t\t📤 Withdrawals : " + with_stat + "\n\t\t\t\t🌲 Channel:" + final + "\n\t\t\t\t💰 Refer: " + refer + "\n\t\t\t\t💰 Minimum: " + mini_with + "\n\t\t\t\t💲 Currency: " + currency + "\n\t\t\t\t🎁 Bonus: " + bonusamount + "\n\t\t\t\t📤 Pay Channel: " + paychannel + "\n\t\t\t\t✏️ Paytm Keys :</b> <code>" + keys + "</code> "
                , { parse_mode: 'html', reply_markup: { inline_keyboard: [[{ text: "💰 Change Refer", callback_data: "refer" }, { text: "💰 Change Minimum", callback_data: "minimum" }], [{ text: "🤖 Bot : " + botstt + "", callback_data: "botstat" }], [{ text: "🌲 Change Channels", callback_data: "channels" }, { text: "🎁 Change Bonus", callback_data: "bonus" }], [{ text: "📤 Withdrawals : " + with_stat + "", callback_data: "withstat" }], [{ text: "🚹 User Details", callback_data: "userdetails" }, { text: "🔄 Change Balance", callback_data: "changebal" }], [{ text: "✏️ Paytm Keys : " + keys + "", callback_data: "keys" }]] } })
        }
    } catch (error) {
...
function sendError(err,ctx){
  bot.telegram.sendMessage(827167974,'<b>😣Error Came From <a href="tg://user?id='+ctx.from.id+'">'+ctx.from.first_name+'</a>: \n</b>'+err+'',{parse_mode:'HTML'})
}
